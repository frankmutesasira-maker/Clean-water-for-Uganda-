...
export default async function handler(req,res){
 ...
 if(typeof reference!=='string'||!['VERIFY','REJECT'].includes(decision)) return res.status(400).json({error:'Reference and valid decision are required.'});
 try{
  const found=await query('select id,amount_usd,status from donations where reference=$1 for update',[reference.trim()]);
  ...
  if(decision==='REJECT'){
   ...
   await query("insert into audit_log(actor,action,entity_type,entity_id,metadata) values($1,'REJECT','donation',$2,$3)",[String(actor).slice(0,100),d.id,JSON.stringify({reference:d.reference})]);
   return res.status(200).json({reference:d.reference,status:'REJECTED'});
  }
  ...
  await query("insert into audit_log(actor,action,entity_type,entity_id,metadata) values($1,'VERIFY','donation',$2,$3)",[String(actor).slice(0,100),d.id,JSON.stringify({reference:d.reference,receipt})]);
  return res.status(200).json({reference:d.reference,status:'VERIFIED',receipt});
