...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!allowedFrequency.has(frequency) || !allowedMethods.has(paymentMethod)) {
      return res.status(400).json({ error: 'Invalid donation options.' });
    }
    if (!/^\S+@\S+\.\S+$/.test(String(donor.email || ''))) {
    ...
    try {
      ...
      const donorResult = await client.query(
        `INSERT INTO donors (first_name, last_name, email, country)
         VALUES ($1,$2,$3,$4)
         ON CONFLICT DO NOTHING
         RETURNING id`,
        [
          cleanString(donor.firstName, 100),
          cleanString(donor.lastName, 100),
          cleanString(donor.email, 320).toLowerCase(),
          cleanString(donor.country, 100) || null
        ]
      );
      ...
      if (projectId) {
        const project = await client.query(
          `SELECT id FROM projects WHERE id=$1 AND published=TRUE LIMIT 1`,
          [projectId]
        );
      ...
      const donation = await client.query(
        `INSERT INTO donations
         (donation_reference, donor_id, project_id, amount, currency, frequency, designation, payment_method)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
         RETURNING id, donation_reference`,
        [reference, donorId, projectId, amount.toFixed(2), currency, frequency, designation, paymentMethod]
      );
      ...
      // Online payment provider integration is intentionally not fabricated.
      // Configure the real provider adapter before enabling live checkout.
      await client.query('ROLLBACK');
      return res.status(503).json({
        error: 'Online checkout is not configured yet. Bank transfer and remittance submission are available.'
      });
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
